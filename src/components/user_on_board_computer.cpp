...
UserOnBoardComputer::UserOnBoardComputer(s2e::environment::ClockGenerator* clock_generator, UserComponents& components)
...
void UserOnBoardComputer::MainRoutine(const int time_count) {
  UNUSED(time_count);
  // Sensor inputs
  const s2e::math::Vector<3> gyro_omega_c = components_.GetGyroSensor().GetMeasuredAngularVelocity_c_rad_s();
